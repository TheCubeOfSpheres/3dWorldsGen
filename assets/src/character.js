...
class ThirdPersonCamera { //Cameras to checkout Horizon ZeroDawn 
  constructor(params) { //Camera and target: Instance of BasicCharacter Controller
    this._params = params;
  ...
  _CalculateIdealOffset() {
    const idealOffset = new THREE.Vector3(-45, 45, -90);
  ...
  Update(timeElapsed) {
    ...
    const t = 1.0 - Math.pow(0.001, timeElapsed); //Allows camera lerp be relatively stable given different FPS
...
class BasicCharacterController {
  constructor(params) {
    this._Init(params); //camera and scene from _LoadAnimatedModel
  }
  ...
  _Init(params) {
   ...
   _LoadModels() {
    ...
    loader.load('Ch34_nonPBR.fbx', (fbx) => {
  ...
  Update(timeInSeconds) {
    ...
    if (this._stateMachine._currentState.Name == 'backflip') {
...
class BackflipState extends State {
  constructor(parent) {
    super(parent);
  ...
  Enter(prevState) {
    const curAction = this._parent._proxy._animations['backflip'].action;
  ...
  _Cleanup() {
    const action = this._parent._proxy._animations['backflip'].action;
